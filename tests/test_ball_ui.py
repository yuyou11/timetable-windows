...
class TestWaitForWindow(unittest.TestCase):
    ...
    def test_returns_as_soon_as_the_window_appears(self):
        ...
        def fake_find(title):
            # ⚠️ 只数**自己这个标题**的调用。
            #
            # `_find` 是按标题找窗口的通用函数，别的代码路径也在调它
            # （`build()` 起的 _polish 线程、悬浮窗落位……），而它们跑在后台
            # 线程里、时机不定。原来这里数的是**所有**调用，于是只要前序测试
            # 留下一个还在跑的线程，计数就多一 —— 表现是「单独跑通过、
            # 全量跑 3 != 4」，看起来像被谁改坏了。
            #
            # 收窄到自己的标题不改变这条测试的原意（「窗口一出现就返回」），
            # 只是不再被无关调用污染。**全局计数器遇上后台线程，必然是脆的。**
            if title != "随便什么标题":
                return 0
            state["calls"] += 1
